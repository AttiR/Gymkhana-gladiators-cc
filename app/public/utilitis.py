...
# Upload image function picture function and videos
...
# Upload Video
def save_video(form_picture):
    random_hex = secrets.token_hex(8)
    _, f_ext = os.path.splitext(form_picture.filename) #f_name=_
    picture_fn = random_hex + f_ext
    picture_path = os.path.join(current_app.root_path, 'static/video/upload', picture_fn)
    form_picture.save(picture_path) 
